"use client"; // this is a client component
import React from "react";
import Image from "next/image";
import { Link } from "react-scroll/modules";
import { HiArrowDown } from "react-icons/hi";
import zech3D from "../public/3d-zech-wave.png";
// import zechResume from "../public/ZechariahResume.pdf";

const HeroSection = () => {
  return (
    <section id="home">
      <div className="flex flex-col text-center items-center justify-center animate-fadeIn animation-delay-2 my-10 py-16 sm:py-24 md:py-40 md:flex-row md:space-x-4 md:text-left">
        <div className="md:mt-1 md:w-1/2">
          <Image
            src={zech3D}
            alt=""
            width={425}
            height={425}
            className="  rounded-3xl"
          />
        </div>
        <div className="md:mt-2 md:w-3/5">
          <h1 className="text-4xl font-bold mt-6 md:mt-0 md:text-6xl">
            Hi, I&#39;m Zechariah!
          </h1>
          <p className="text-lg mt-4 mb-6 md:text-2xl">
            I&#39;m a{" "}
            <span className="font-semibold text-teal-600">
              Frontend Engineer{" "}
            </span>
            based in Lagos, Nigeria. I enjoy solving users problems and turning
            designs into user friendly applications, Working towards creating
            software that makes life easier and more meaningful.
          </p>
          <Link
            to="contact"
            className="text-neutral-100 font-semibold px-6 py-3 bg-teal-600 rounded shadow hover:bg-teal-700 cursor-pointer"
            activeClass="active"
            spy={true}
            smooth={true}
            offset={-100}
            duration={500}
          >
            Hire Me
          </Link>
        </div>
      </div>
      <div className="flex flex-row items-center text-center justify-center ">
        <Link
          to="about"
          activeClass="active"
          spy={true}
          smooth={true}
          offset={-100}
          duration={500}
        >
          <HiArrowDown size={35} className="animate-bounce" />
        </Link>
      </div>
    </section>
  );
};

export default HeroSection;
